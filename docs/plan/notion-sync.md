...
- 큰 문서의 블록/요청 수 최적화: 여전히 변경 파일 전체 본문을 재작성한다(diff 부분 갱신은 미구현). 단, 거대 문서(worklog 등)에서 본문 교체 시 블록을 **하나씩 순차 삭제**해 타임아웃이 나던 문제는 해결 — 블록 삭제를 **동시성 제한 병렬화**하고 모든 Notion 호출에 **재시도(타임아웃·429·5xx 지수 백오프) + 클라이언트 timeoutMs 상향**을 적용. 근본적인 diff 기반 부분 갱신/문서 분할은 후속.
