...
- 단순 GET 두 경로를 **대비**해, "DB 의존이 더하는 지연"을 분리해서 본다.
  ...
  - **조회+DB** `GET /api/grass/:token` — `get_grass` RPC + SVG 렌더. 실제 읽기 경로(DB 왕복).
- 둘 다 인증이 필요 없어 시크릿/세션 없이 재현 가능하다(미들웨어에서 `health`·`api/grass` 제외).
...
| `grass_read` (조회+DB) | _ | _ | _ | _ | _ | _ |

## 병목 가설

- **DB 왕복이 지연의 지배 요인일 것**: `grass_read` p95 − `health_live` p95 ≈ `get_grass` RPC + 직렬화/렌더 비용. 이 차이가 크면 DB(인덱스·RPC 쿼리·커넥션)부터 본다.
- **개선 후보**: ① `get_grass` 쿼리/인덱스 점검, ② 결과 캐시(공개 임베드라 짧은 TTL 캐시 여지), ③ SVG 렌더 비용(서버 CPU) 분리, ④ DB 커넥션 풀.
