...
원본 k6 출력은 `docs/performance/results/`에 커밋돼 있다 — `baseline.*`(개선 전, 100% 실패), `after3.*`(개선 후 좋은 구간, 통과), `after5.*`(나쁜 구간, 2s fail-fast), `after-vus5.*`(저부하). `*.txt`는 콘솔 그대로, `*.json`은 전체 메트릭.
