...
//   ② metrics_read : GET /metrics       — metrics_snapshot RPC, 실제 읽기 경로(DB 왕복). 인증/토큰 불필요.
...
export function metricsRead() {
  const res = http.get(`${BASE}/metrics`, { tags: { scenario: "metrics_read" } });
  check(res, {
    "metrics_read 200": (r) => r.status === 200,
    "metrics_read has gauge": (r) => r.body && r.body.includes("dailyproof_jobs_total"),
  });
}
